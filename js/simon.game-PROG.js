// Simon Game in 33 lines of JavaScript code -- no jquery
// get the 4 buttons in simon-container into an array (Node List)
...
function serveBtn() {
    ...
    setTimeout(() => { // 1.0 sec delay then rand btn vanishes
        b.style.opacity = "0";
        new Audio(`audio/${b.id}.mp3`).play();
        setTimeout(() => b.style.opacity = "1", 600); // 0.6 sec later, button reappears
    }, 1000);
